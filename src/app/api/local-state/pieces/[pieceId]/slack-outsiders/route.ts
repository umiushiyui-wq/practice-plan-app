...
import { joinConversation, listConversationMembers } from "@/lib/slack";
...
export async function POST(request: Request, context: { params: Promise<{ pieceId: string }> }) {
  try {
    ...
    const pieceMemberIdSet = new Set(piece.memberIds);
    ...
    const channelMemberSet = new Set(channelMembers.members);
    const outsiders = members
      .filter((member) => member.slackUserId && channelMemberSet.has(member.slackUserId))
      .filter((member) => !pieceMemberIdSet.has(member.id))
      .map((member) => ({ memberId: member.id, name: member.name, slackUserId: member.slackUserId }));

    return NextResponse.json({ ok: true, outsiders });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "退出候補の取得に失敗しました。" },
      { status: 500 }
    );
  }
}
