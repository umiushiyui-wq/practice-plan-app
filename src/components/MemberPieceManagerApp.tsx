...
const SETUP_SECTIONS = [
  ...
  { id: "members", label: "奏者の追加" },
  { id: "pieces", label: "曲の追加" }
] as const;
...
export function MemberPieceManagerApp() {
  ...
  useEffect(() => {
    const syncSectionFromHash = () => setActiveSetupSection(readSetupSectionFromHash());
  ...
  function selectSetupSection(sectionId: SetupSectionId) {
  ...
  return (
    <main className="stack setup-page">
      ...
      <nav className="setup-section-tabs" aria-label="追加項目">
        {SETUP_SECTIONS.map((section) => (
          <button
            className={`secondary setup-section-tab${activeSetupSection === section.id ? " is-active" : ""}`}
            key={section.id}
            type="button"
            onClick={() => selectSetupSection(section.id)}
          >
            {section.label}
          </button>
        ))}
      ...
      <div className="stack">
        ...
        {activeSetupSection === "practice-days" ? (
